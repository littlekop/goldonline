...
4. **Generate a cover image.** This is the site's locked cover-image design (dark scrim over a real Pexels photo, logo, category tag, a green/red/gold trend-chart motif, and the headline baked in) — do not redesign it, only feed it the right inputs. Run (from `web/`):
   ```
   cd web && node scripts/generate-article-cover.mjs "<article title>" <slug> "<english bg-image query>" <up|down|flat> "<tag>"
   ```
   - `<article title>`: the exact `title` you're putting in frontmatter.
   - `<english bg-image query>`: a short, visually concrete Pexels search matched to the story's *specific* subject, not a generic "gold bars" every time — e.g. "federal reserve building" for a Fed story, "iran flag protest" for a geopolitical piece, "businessman stock chart" for a market-reaction piece, "gold bars macro" only when the story really is just about the metal itself. Vary this — the script already avoids repeating the exact same photo, but a well-matched query makes the image relevant, not just unique.
   - `<up|down|flat>`: whether *this article's* core price move is up, down, or unchanged — drives the chart's color (green/red/gold). Base it on the actual number you're reporting, not vibes.
   - `<tag>`: short category label shown in the image's pill, e.g. `ข่าวทองคำ`, `วิเคราะห์ราคาทอง` — match your frontmatter `tags[0]` where sensible.
   It prints `{coverImage, coverImageCredit, sourcePage}` — put `coverImage`/`coverImageCredit` straight into frontmatter. Requires `PEXELS_API_KEY` in `web/.env.local` (it self-loads the file); if unset, the script errors — in that case skip this step (omit `coverImage`/`coverImageCredit`) rather than blocking, and note the skip in your log entry.
