...
1. **Research — RSS first, then verify.** Run:
   ...
   ```
   (pass a different hour count to match your actual check interval, e.g. `1` for an hourly schedule). Pulls recent items from Google News (English "gold price", Thai "ราคาทอง", "gold market analysis") as JSON: `{title, link, source, pubDate}`, no API key needed. Skim for anything genuinely price-move-worthy. If nothing warrants an article, stop here and log a "nothing newsworthy" entry (see Logging below) — don't force a low-value article. Otherwise, WebFetch the 2-4 most relevant `link`s and read the full articles before writing — never cite a headline you haven't opened. Supplement with WebSearch if RSS doesn't surface enough on the angle you need.
