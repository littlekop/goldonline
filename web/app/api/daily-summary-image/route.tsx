...
export async function GET(req: NextRequest) {
  ...
  const Stat = ({ label, value }: { label: string; value: string }) => (
    <div style={{ display: "flex", flexDirection: "column", flex: 1 }}>
      <div style={{ fontSize: 24, color: "#6b5a2e", display: "flex" }}>{label}</div>
      <div style={{ fontSize: 56, fontWeight: 700, color: "#211a0e", display: "flex" }}>{value}</div>
    </div>
  );
  ...
  return new ImageResponse(
    (
      ...
      >
        ...
        <div style={{ fontSize: 60, fontWeight: 700, color: "#211a0e", marginTop: 28, display: "flex" }}>
          สรุปราคาทองคำวันนี้
        </div>
        <div style={{ fontSize: 26, color: "#6b5a2e", marginTop: 6, display: "flex" }}>
          ทองคำแท่ง 96.5% · {date}
        </div>
